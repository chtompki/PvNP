...
% Do not cite any references in the abstract.
    ...
    \begin{proof}
        ...
        equal. So we set:
        \begin{eqnarray*}
            \text{deterministic polynomial time algorithms} & = & \text{non-deterministic polynomial time algorithms} \\
            \text{(deterministic polynomial time algorithms)} & = & \text{non-(deterministic polynomial time algorithms)} \\
            \text{(deterministic polynomial time algorithms)} & = & \text{not-(deterministic polynomial time algorithms)} \\
            q & = & \text{not } q,
        \end{eqnarray*}
        where $q=\text{``deterministic polynomial time algorithms''}$. But $q = \text{not } q$ is simply the liar's paradox,
